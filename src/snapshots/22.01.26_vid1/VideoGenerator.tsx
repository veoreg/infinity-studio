...
// -----------------------------------------------------------------------------------------
const RecentMasterpieceItem = ({ item, onViewFull, onDelete }: { item: any; onViewFull: (url: string) => void; onDelete: (id: string, url: string) => void }) => {
        ...
        <div className="group/item relative bg-[#080808] border border-[#d2ac47]/10 rounded-2xl overflow-hidden aspect-square shadow-2xl transition-all hover:border-[#d2ac47]/40 hover:-translate-y-1">
            ...
            <div className="absolute inset-0 z-20 pointer-events-none opacity-0 group-hover/item:opacity-100 transition-all duration-500">
                ...
                <button
                    onClick={(e) => {
                        e.stopPropagation();
                        if (window.confirm('Delete this masterpiece?')) onDelete(item.id, item.video_url);
                    }}
                        ...
                        <div className="absolute inset-0 w-full h-full bg-[#050505] flex items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-[#1a1a1a] to-[#050505] rounded-2xl">
                            ...
                            {loading ? (
                                ...
                                <div className="relative w-full h-full flex items-center justify-center group">
                                    ...
                                    {/* Premium Control Overlay Bar (Functional) */}
                                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-[90%] md:w-[80%] h-14 bg-black/60 backdrop-blur-xl border border-[#d2ac47]/20 rounded-2xl flex items-center px-4 gap-4 shadow-[0_0_30px_rgba(0,0,0,0.5)] opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0 pointer-events-auto">
                                        <button
                                            onClick={() => {
                                                if (videoRef.current) {
                                            ...
                                            className="w-8 h-8 rounded-full bg-[#d2ac47]/20 flex items-center justify-center cursor-pointer hover:bg-[#d2ac47] hover:text-black transition-all group/play"
                                        >
                                            <Play size={12} className="text-[#d2ac47] fill-[#d2ac47] group-hover/play:text-black group-hover/play:fill-black" />
                                        </button>

                                        {/* Scrubber - Functional */}
                                        <div
                                            className="flex-1 h-1.5 bg-white/10 rounded-full relative overflow-hidden group/scrub cursor-pointer"
                                            onClick={handleSeek}
                                        >
                                            {/* Progress Bar */}
                                            <div
                                                className="absolute inset-y-0 left-0 bg-gold-gradient shadow-[0_0_15px_rgba(210,172,71,0.6)] transition-all duration-100 ease-linear"
                                                style={{ width: `${videoProgress}%` }}
                                            ></div>
                                            {/* Scrubber Knob */}
                                            <div
                                                className="absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-white rounded-full shadow-[0_0_10px_white] transition-all duration-100 ease-linear group-hover/scrub:scale-125"
                                                style={{ left: `${videoProgress}%` }}
                                            ></div>
                                        </div>

                                        <div className="flex items-center gap-3 text-[10px] text-[#d2ac47] font-bold tracking-widest uppercase">
                                            <span>4K</span>
                                            <div className="w-1.5 h-1.5 rounded-full bg-[#d2ac47]/30"></div>
                                            <span>60FPS</span>
                                            <button onClick={toggleFullscreen} className="hover:text-white transition-colors">
                                                <Maximize2 size={12} className="opacity-70 hover:opacity-100 cursor-pointer" />
                                            </button>
                                        </div>
                                    </div>
